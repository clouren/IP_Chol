...
)
{
    if (!A || !pargin)
    {
        IP_mex_error (SLIP_INCORRECT_INPUT);
    }
    ...
    if (!Ai_int || !Ap_int) {IP_mex_error(SLIP_OUT_OF_MEMORY);}

    // Does x have inf?
    IP_mex_check_for_inf(Ax, Anz);
    IP_mwIndex_to_int32(Ai_int, Ai, Anz);
    IP_mwIndex_to_int32(Ap_int, Ap, nA+1);
    ...
    if (mxGetScalar(tmp) > 0) 
    {
        ...
        if (!Ax_int) {IP_mex_error(SLIP_OUT_OF_MEMORY);}
        for (k = 0; k < Anz; k++)
        {
    ...
    if (nargin == 3)
    {
        ...
        if (mxGetScalar(tmp) > 0)
        {
            ...
            if (!bx_int) {IP_mex_error(SLIP_OUT_OF_MEMORY);}
            for (k = 0; k < nb; k++)
            {
                for (j = 0; j < mb; j++)
                {
        ...
        else
        {
            ...
            if (!bx_doub) {IP_mex_error(SLIP_OUT_OF_MEMORY);}
